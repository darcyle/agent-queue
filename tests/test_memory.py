...
# ---------------------------------------------------------------------------
# Hook engine memory_search context step tests
# ---------------------------------------------------------------------------

class TestMemorySearchHookStep:
    """Tests for the ``memory_search`` context step type in HookEngine.

    These tests mock the orchestrator, database, and MemoryManager so they
    run without any external dependencies.
    """

    @pytest.fixture
    def engine(self):
        """Create a minimal HookEngine with mocked dependencies."""
        ...
        eng = HookEngine(db, bus, cfg)
        eng._orchestrator = MagicMock()
        eng._orchestrator.memory_manager = MagicMock()
        eng._orchestrator.memory_manager.search = AsyncMock(return_value=[])
        return eng

    async def test_memory_search_step_basic(self, engine):
        """Basic memory_search step returns formatted content."""
        engine._orchestrator.memory_manager.search = AsyncMock(return_value=[
            {"source": "tasks/auth.md", "heading": "Auth Module", "content": "JWT auth impl", "score": 0.92},
            {"source": "notes/arch.md", "heading": "Architecture", "content": "We use PostgreSQL", "score": 0.81},
        ])
        engine.db.get_project_workspace_path = AsyncMock(return_value="/tmp/ws")

        step = {"type": "memory_search", "project_id": "my-proj", "query": "authentication", "top_k": 5}
        ...
        assert "JWT auth impl" in result["content"]
        assert "Architecture" in result["content"]
        assert "0.920" in result["content"]

        engine._orchestrator.memory_manager.search.assert_called_once_with(
            "my-proj", "/tmp/ws", "authentication", top_k=5
        )

    async def test_memory_search_step_empty_results(self, engine):
        """Empty search results return empty content."""
        engine._orchestrator.memory_manager.search = AsyncMock(return_value=[])
        engine.db.get_project_workspace_path = AsyncMock(return_value="/tmp/ws")

        step = {"type": "memory_search", "project_id": "proj", "query": "nonexistent"}
        results = await engine._run_context_steps([step])

        assert results[0]["content"] == ""
        assert results[0]["count"] == 0

    async def test_memory_search_step_no_orchestrator(self, engine):
        """Graceful degradation when orchestrator has no memory_manager."""
        engine._orchestrator.memory_manager = None

        step = {"type": "memory_search", "project_id": "proj", "query": "test"}
        results = await engine._run_context_steps([step])

        assert results[0]["content"] == ""
        assert results[0]["count"] == 0

    async def test_memory_search_step_no_workspace(self, engine):
        """Returns empty content when project has no workspace."""
        engine.db.get_project_workspace_path = AsyncMock(return_value=None)
    ...
    async def test_memory_search_step_search_error(self, engine):
        """Graceful degradation when memory search raises an exception."""
        engine._orchestrator.memory_manager.search = AsyncMock(
        ...
        assert results[0]["content"] == ""
        assert "error" in results[0]
        assert "search boom" in results[0]["error"]

    async def test_memory_search_step_missing_project_id(self, engine):
        """Returns error when project_id is missing."""
        step = {"type": "memory_search", "query": "test"}
        results = await engine._run_context_steps([step])

        assert results[0]["content"] == ""
        assert "error" in results[0]

    async def test_memory_search_step_missing_query(self, engine):
        """Returns error when query is missing."""
        step = {"type": "memory_search", "project_id": "proj"}
        results = await engine._run_context_steps([step])

        assert results[0]["content"] == ""
        assert "error" in results[0]

    async def test_memory_search_step_default_top_k(self, engine):
        """Default top_k is 3 when not specified."""
        engine._orchestrator.memory_manager.search = AsyncMock(return_value=[])
        engine.db.get_project_workspace_path = AsyncMock(return_value="/tmp/ws")

        step = {"type": "memory_search", "project_id": "proj", "query": "test"}
        await engine._run_context_steps([step])

        _, kwargs = engine._orchestrator.memory_manager.search.call_args
        assert kwargs["top_k"] == 3

    async def test_memory_search_step_resolves_event_placeholders(self, engine):
        """Template placeholders in project_id and query are resolved from event data."""
        engine._orchestrator.memory_manager.search = AsyncMock(return_value=[])
        engine.db.get_project_workspace_path = AsyncMock(return_value="/tmp/ws")
        ...
        event_data = {"project_id": "resolved-proj", "description": "resolved query text"}
        results = await engine._run_context_steps([step], event_data)

        engine._orchestrator.memory_manager.search.assert_called_once_with(
            "resolved-proj", "/tmp/ws", "resolved query text", top_k=3
        )

    async def test_memory_search_step_in_pipeline(self, engine):
        """memory_search works alongside other step types in a pipeline."""
        engine._orchestrator.memory_manager.search = AsyncMock(return_value=[
            {"source": "test.md", "heading": "Test", "content": "memory content", "score": 0.9},
        ])
        engine.db.get_project_workspace_path = AsyncMock(return_value="/tmp/ws")

        steps = [
            {"type": "shell", "command": "echo hello", "timeout": 5},
            {"type": "memory_search", "project_id": "proj", "query": "context"},
        ]
        results = await engine._run_context_steps(steps)

        assert len(results) == 2
        assert "hello" in results[0]["stdout"]
        assert "memory content" in results[1]["content"]

    async def test_memory_search_step_markdown_formatting(self, engine):
        """Verify the formatted output contains ranked entries with scores."""
        engine._orchestrator.memory_manager.search = AsyncMock(return_value=[
            {"source": "a.md", "heading": "First", "content": "Content A", "score": 0.95},
            {"source": "b.md", "heading": "", "content": "Content B", "score": 0.80},
        ])
        engine.db.get_project_workspace_path = AsyncMock(return_value="/tmp/ws")
        ...
        # First entry has heading
        assert "[1] First" in content
        assert "0.950" in content
        assert "Content A" in content
        # Second entry has no heading, falls back to source
        assert "[2] (from b.md)" in content
        assert "0.800" in content
        assert "Content B" in content
        # Entries are separated by divider
        assert "---" in content

    async def test_memory_search_step_workspace_lookup_error(self, engine):
        """Graceful handling when workspace lookup raises."""
        engine.db.get_project_workspace_path = AsyncMock(

...
from src.models import (
    Agent, AgentState, DashboardConfig, Hook, Project, ProjectStatus,
    RepoSourceType, Task, TaskStatus, TaskType, VerificationType,
    TASK_TYPE_VALUES, WidgetPrivacyConfig, WidgetVisibility, Workspace,
)
...
class CommandHandler:
    ...
    # -----------------------------------------------------------------------
    # Dashboard config commands -- per-project widget privacy settings.
    # Each project can have one dashboard configuration that specifies an
    # owner and per-widget visibility (visible / owner_only / collapsed).
    # -----------------------------------------------------------------------

    async def _cmd_get_dashboard_config(self, args: dict) -> dict:
        """Retrieve the dashboard configuration for a project.

        Returns the full config including widget privacy settings.
        """
        project_id = args.get("project_id") or self._active_project_id
        if not project_id:
            return {"error": "project_id is required"}
        config = await self.db.get_dashboard_config(project_id)
        if not config:
            return {"config": None, "project_id": project_id}
        return {
            "config": {
                "id": config.id,
    ...
    async def _cmd_set_dashboard_owner(self, args: dict) -> dict:
        """Set or create the dashboard owner for a project.

        If no dashboard config exists yet, creates one with the given
        owner_user_id.  Otherwise updates the existing owner.
        """
        project_id = args.get("project_id") or self._active_project_id
        if not project_id:
            return {"error": "project_id is required"}
        ...
        existing = await self.db.get_dashboard_config(project_id)
        if existing:
            await self.db.update_dashboard_config(
            ...
            config = DashboardConfig(
                id=f"dash-{_uuid.uuid4().hex[:8]}",
                project_id=project_id,
                owner_user_id=owner_user_id,
            )
            await self.db.create_dashboard_config(config)
            return {
                "status": "created",
                "project_id": project_id,
                "owner_user_id": owner_user_id,
    ...
    async def _cmd_set_widget_privacy(self, args: dict) -> dict:
        """Set the visibility of a specific widget for a project.

        Creates the dashboard config if it doesn't exist (requires
        owner_user_id in that case).  Updates the widget's visibility
        setting within the config's widget_configs list.

        Args:
            project_id: Target project
            widget_id: Widget identifier (e.g. "budget", "token_usage",
                "task_progress", "task_list")
            visibility: One of "visible", "owner_only", "collapsed"
            placeholder_text: Optional custom placeholder for owner_only
            owner_user_id: Required when creating a new dashboard config
        """
        project_id = args.get("project_id") or self._active_project_id
        if not project_id:
            return {"error": "project_id is required"}
        ...
        visibility_str = args.get("visibility", "owner_only")
        try:
            visibility = WidgetVisibility(visibility_str)
        except ValueError:
            valid = ", ".join(v.value for v in WidgetVisibility)
            return {"error": f"Invalid visibility '{visibility_str}'. Valid: {valid}"}

        placeholder_text = args.get(
            "placeholder_text", "🔒 This widget is private",
        )
        ...
        if not config:
            ...
            if not owner_user_id:
                return {
                    "error": (
                        "No dashboard config exists for this project. "
                        "Provide owner_user_id to create one, or use "
                        "set_dashboard_owner first."
                    ),
                }
            import uuid as _uuid
            config = DashboardConfig(
        ...
        # Update or add the widget config
        new_wc = WidgetPrivacyConfig(
            widget_id=widget_id,
            visibility=visibility,
            placeholder_text=placeholder_text,
        )
        updated_widgets = [
            wc for wc in config.widget_configs
            if wc.widget_id != widget_id
        ]
        updated_widgets.append(new_wc)

        await self.db.update_dashboard_config(
            project_id, widget_configs=updated_widgets,
        )
        return {
            "status": "updated",
            "project_id": project_id,
    ...
    async def _cmd_remove_widget_privacy(self, args: dict) -> dict:
        """Remove privacy settings for a specific widget (make it visible).

        Removes the widget from the config's widget_configs list,
        effectively making it visible to all authorized users.
        """
        project_id = args.get("project_id") or self._active_project_id
        if not project_id:
            return {"error": "project_id is required"}
        ...
        config = await self.db.get_dashboard_config(project_id)
        if not config:
            return {"error": f"No dashboard config for project '{project_id}'"}

        updated_widgets = [
            wc for wc in config.widget_configs
            if wc.widget_id != widget_id
        ]
        if len(updated_widgets) == len(config.widget_configs):
            return {"error": f"Widget '{widget_id}' has no privacy settings"}
    ...
    async def _cmd_list_widget_privacy(self, args: dict) -> dict:
        """List all widget privacy settings for a project."""
        project_id = args.get("project_id") or self._active_project_id
        if not project_id:
            return {"error": "project_id is required"}

    ...
    async def _execute_task(self, action: AssignAction) -> None:
        ...
        if task.is_plan_subtask:
            ...
            context_lines.append(
                ...
                "2. `git commit` with a descriptive message\n"
                "Do NOT push — the system handles pushing and PR creation.\n"
                "\n## Writing Implementation Plans (only if your task requires it)\n"
                ...
                "\n"
                "If you DO need to write a plan:\n"
                "- Write it to `.claude/plan.md` or `plan.md` in the workspace root\n"
                "- Do NOT write plans to other locations (e.g. `docs/`, `notes/`) — the system\n"
                "  only searches specific paths for automatic task splitting\n"
                "- Use `## Phase N: <title>` headings for each step\n"
                "- Put background/reference material BEFORE the phase headings"
            )

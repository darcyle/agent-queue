...
@dataclass
class ChatAnalyzerSuggestion:
    """A suggestion generated by the background chat analyzer.

    Stored in the ``chat_analyzer_suggestions`` table. The analyzer creates
    suggestions when it detects something useful in project channel
    conversations. Users accept or dismiss suggestions via Discord buttons,
    which updates the status and resolved_at fields.
    """
    ...
    context_snapshot: str | None = None  # JSON of recent messages at time of suggestion

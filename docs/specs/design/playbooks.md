...
Periodic triggers (timers) are modeled as synthetic events — e.g., `timer.30m` — so
that all playbooks share the same event-driven execution model.
...
Periodic triggers (every 30 minutes, every 4 hours) are modeled as synthetic timer
events so that all playbooks share a uniform event-driven model:

- `timer.{interval}` events (e.g., `timer.30m`, `timer.4h`, `timer.24h`)
- Generated by a lightweight timer service in the orchestrator loop
- Carry minimal payload: `{"tick_time": "...", "interval": "..."}`
- Playbooks subscribe to these like any other event
...
Timer events carry `project_id: null` — they are inherently system-scoped. A
project-scoped playbook can trigger on a timer, but it fires once globally, not
once per project.
...
- **Timer service:** Scans compiled playbooks for intervals, emits only subscribed
  timers (Section 7)
